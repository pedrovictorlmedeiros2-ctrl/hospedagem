...
| 4 | Economia (coins) sujeita a duplicação/race condition | Alto — quebra a integridade do produto | Ledger append-only (`WalletTransaction`) com `idempotencyKey` único; nunca mutar saldo diretamente (ver prisma/schema.prisma) | Mitigado para o que existe hoje (recompensa de partida + treino intensivo) — `WalletRepository.applyTransaction` testado para idempotência em chamada repetida e em corrida real (`Promise.all`), e para rejeição de saldo negativo sem mutar o saldo. Reabre quando mercado/transferências/contratos (Fase 6, escopo restante) forem implementados — cada fonte/sumidouro novo precisa do mesmo tratamento |
...
| 20 | Pesos de recompensa de partida (`economy/domain/matchReward.ts`) são heurística ajustada "no olho", não calibrada com dados reais de economia | Médio — balanceamento de coins pode estar longe do ideal (farm de banco vs titular, inflação ao longo de uma temporada) | Documentado no código e aqui; mesma categoria de risco já aceita para os pesos de IA do motor (Risco #12); revisar quando houver dados reais de uso | Aceito por ora, esperado para um v1 |
| 21 | Escopo da Fase 6 do plano original ("Coins; recompensas; mercado; transferências; contratos") só foi parcialmente implementado — mercado, transferências e contratos não têm nenhuma lógica de aplicação, só o schema (`Transfer`/`Contract`) | Nenhum hoje (nada usa esses modelos) — mas é trabalho pendente real, não concluído | Decisão de escopo registrada em ADR 0001 (adenda Fase 6) e ROADMAP.md; retomar como continuação da Fase 6 antes de declarar a fase "completa" | Aceito conscientemente, não esquecido |
| 22 | Segunda moeda (`Wallet.tokens`) não tem nenhuma fonte ou sumidouro implementado — só existe porque o schema já a modelava | Nenhum (campo sempre fica em 0) | Fica sem uso até haver uma feature real que precise de uma moeda premium separada | Aceito por ora |
