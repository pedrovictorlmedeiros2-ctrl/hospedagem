...
## Adenda (follow-up) — Revisão de segurança/concorrência da economia: 3 races confirmadas e corrigidas

Pedido explícito do usuário após o follow-up de Conquistas: "faz alguns testes só pra ver se a bugs e alguns de segurança pra ver se não tem como quebrar a economia". Metodologia: auditoria de todo ponto de construção de `idempotencyKey` no codebase, seguida de scripts throwaway (`npx tsx`, apagados depois de extrair o achado — mesma convenção já usada pra brute-force de seeds determinísticos) disparando chamadas genuinamente concorrentes via `Promise.all`/`Promise.allSettled` contra os adapters in-memory, que interleiam de verdade em Node por causa dos múltiplos pontos de `await` nos serviços. `respondToDuel` (Fase 8) foi usado como controle positivo — sua transição de estado guardada PENDING→FINISHED foi reconfirmada segura sob corrida, servindo de modelo de referência ("claim atômico antes de qualquer mutação") pro resto da auditoria.

Três races reais confirmadas e corrigidas (ver Riscos #45–47 em RISK_REGISTER.md para o detalhe completo de cada uma):

- **`MatchRepository.persistMatchResult`** não tinha nenhuma proteção de idempotência em `existingMatchId` — a mais severa das três, porque é a fronteira de integridade de dado mais central do produto (o resultado de partida) e, ainda assim, tinha MENOS proteção que qualquer outro caminho de escrita do sistema. Corrigida com dedupe por `existingMatchId`: check síncrono em `InMemoryMatchRepository`; pré-check em `MatchPlayerStat` (`@@unique([matchId, playerId])`) + captura de `P2002` como defesa em profundidade em `PrismaMatchRepository`, mesmo padrão de `PrismaWalletRepository.applyTransaction`.
- **Bônus de assinatura de transferência** (`acceptTransferOffer`) driblava o cooldown de 30 dias inteiro quando duas transferências concorrentes miravam clubes DIFERENTES — a `idempotencyKey` do bônus é escopada por clube de destino, então não protegia contra esse caso. Corrigida com um claim atômico dedicado (`Career.lastTransferClaimAt`, compare-and-swap via `UPDATE ... WHERE ... IS NULL OR <= cutoff`), separado da idempotencyKey da carteira.
- **Cooldown de treino** (`trainPlayer`) tinha o mesmo formato de race — check-then-act com vários `await`s entre a leitura e a escrita. Corrigida com o mesmo padrão de claim atômico, em `Player.lastTrainingClaimAt` (não em `Career`, porque treinar não exige que uma carreira já exista).

Decisão de design comum às três correções, generalizando o padrão já usado em `respondToDuel`: **o claim atômico é sempre a ÚLTIMA porta antes da mutação irreversível, nunca a primeira.** Toda checagem que pode falhar por conta própria (estamina, lesão ativa, saldo insuficiente, foco inválido pra posição) roda ANTES do claim — se o claim rodasse primeiro, uma falha nessas checagens desperdiçaria o cooldown do jogador por nada (uma tentativa que nunca treinou/transferiu de verdade, mas queima o dia inteiro). Isso significa que a ordem de checagem mudou ligeiramente em `trainPlayer` (estamina agora é checada antes do claim de cooldown, não depois) — sem mudança visível de comportamento fora de corrida real.
...
## Adenda (follow-up) — Partida interativa: decisão tática no intervalo, sem persistência de estado pausado

Pedido direto de um usuário testando o bot pela primeira vez: "achei 100% comandos e não tem muitas coisas igual um FIFA". Depois de mapear o pedido maior (decisão em tempo real x competições reais) e escolher decisão em tempo real como ponto de partida, esta adenda cobre a fatia mínima escolhida: **um único ponto de decisão, no intervalo de `/jogar-carreira`**, não decisão a cada lance (pênalti, substituição pontual) — cada um desses exigiria seu próprio ponto de pausa no motor, fora do escopo deste v1.

Decisões de arquitetura:

- **O motor já era estruturado em "primeiro tempo → intervalo → segundo tempo" internamente** (`simulateMatch`), então a divisão em `simulateFirstHalf`/`simulateSecondHalf` (`game/engine/simulateMatch.ts`) não exigiu nenhuma lógica nova — só expôs o ponto de corte que já existia. `simulateMatch` continua existindo como composição das duas (`runSecondHalf(runFirstHalf(...))`), byte-idêntica a antes da divisão para o mesmo seed — confirmado pelos 9 testes de determinismo originais passando sem nenhuma alteração, mais um teste novo comparando a composição contra a versão monolítica diretamente.
- **Nenhum mecanismo novo de simulação foi inventado para a tática.** O motor já tinha `TeamStyle` (Defensivo/Agressivo/Equilibrado/etc.) influenciando de verdade os pesos de decisão da IA (`game/ai/decide.ts`) desde a Fase 3 — a escolha do intervalo (`game/domain/tactics.ts`) só mapeia pra esse mecanismo existente via `styleForHalftimeTactic`, mutando `firstHalf.state[playerSide].squad.style` entre as duas chamadas. `BALANCED` mapeia pro mesmo `TACTICAL` que `buildSquadFromProfile` já usava como padrão — escolher Equilibrado (ou não ter o hook de decisão) produz resultado idêntico ao comportamento pré-existente.
- **`playCareerMatch` ganhou um dependency hook opcional (`resolveHalftimeTactic`), não uma segunda função paralela.** Todo o pós-processamento existente (recompensa, salário, lesão, progressão de estágio, conquistas) continua um fluxo único — só um `await` a mais no meio, chamado apenas quando o hook está presente. Todos os 16 testes originais de `playCareerMatch.test.ts` continuam passando sem nenhuma alteração, confirmando que omitir o hook é 100% equivalente ao comportamento anterior.
- **Sem tabela nova no banco, sem persistência de estado pausado.** A espera pelo clique acontece inteiramente em memória, dentro da mesma execução assíncrona do comando, via `Message.awaitMessageComponent` do discord.js (timeout de 5 minutos, cai pra Equilibrado se ninguém clicar). Isso só é seguro porque `MatchSimState` (com `Map`s e um RNG que é uma closure) nunca precisa sair do processo — a alternativa (persistir e resumir em outra invocação) exigiria repensar a serialização do estado do motor inteiro, desproporcional para uma única decisão binária por partida. Ver Risco #50 no trade-off de reinício do processo no meio da espera.
- **Primeiro uso de botões/interações de componente no bot** — até aqui, todo comando era só request/response de slash command. `discord/client.ts` não precisou de nenhuma mudança: o listener de `InteractionCreate` já ignora tudo que não é `isChatInputCommand()`, e o collector do `awaitMessageComponent` escuta de forma independente, escopado à mensagem específica do intervalo.

**O que foi validado de verdade:** 5 testes novos (378 no total) — 3 no motor (`simulateFirstHalf`/`simulateSecondHalf` compõem pro mesmo resultado que `simulateMatch`; `simulateFirstHalf` para exatamente no intervalo; mutar o estilo entre as duas metades muda o resultado de verdade, comparação determinística não estatística) e 2 no serviço (o hook é chamado exatamente uma vez com o placar/lado corretos, e Equilibrado é um no-op de verdade comparado a omitir o hook; Ofensivo produz uma sequência de eventos diferente de Equilibrado). Card do intervalo renderizado com dados de exemplo via script throwaway antes de apagar. `npx tsc --noEmit`, `npx eslint .`, `npx vitest run` (378 passando) e `npm run build` — todos limpos.

## Adenda (follow-up) — Pênalti interativo: pausa DENTRO de um minuto, não só entre minutos

A adenda anterior deixou explícito que "decisão a cada lance (pênalti, substituição pontual)... exigiria seu próprio ponto de pausa no motor, fora do escopo deste v1". Pedido do usuário via `AskUserQuestion` para escolher entre duas abordagens ("preferência definida no intervalo" vs. "botão aparece na hora exata do lance") — escolheu a segunda, mais fiel à ideia original, topando o escopo maior.

O motor até aqui só pausava **entre** minutos inteiros (`simulateFirstHalf`/`simulateUntilMinute` — ver adenda anterior). Um pênalti é decidido e aplicado inteiramente **dentro** de uma única chamada de `resolvePhase`/`applyOutcome` (o caso `FOUL`), sem nenhum ponto intermediário onde o motor "avisa" que vai rolar um pênalti antes de já ter decidido o resultado — pausar exigiu mexer nessa resolução em si, não só embrulhar chamadas existentes.

Decisões de arquitetura:

- **`MatchSimState.pendingPenalty`** (`game/domain/state.ts`) é o novo mecanismo de pausa intra-minuto: quando `MatchOptions.pauseOnPenalty` está ligado, `applyOutcome` (caso `FOUL`, `resolvePhase.ts`) captura `{ minute, attackingSide, taker, gk }` nesse campo e retorna **sem** decidir o pênalti (sem gol, sem evento, sem resetar posse/zona) em vez de chamar `resolvePenalty` na hora. `runMinutesUntil` (novo helper interno em `simulateMatch.ts`, substituindo o loop duplicado de `runFirstHalf`/`runUntilMinute`) checa esse campo a cada minuto e para o loop imediatamente — o resto do minuto (dreno de estamina, substituição por cansaço) já rodou normalmente, só a resolução do próprio pênalti fica pendente.
- **`resumePendingPenalty`** (novo, exportado de `simulateMatch.ts`) resolve o pênalti pendente (ou não faz nada se não houver nenhum) e limpa o campo. **`resumeFirstHalf`** (novo) continua rodando minutos até 45 e só então fecha a burocracia do intervalo (evento `HALFTIME` + recuperação de estamina) — sem essa separação, o intervalo seria fechado cedo demais se um pênalti pausasse a simulação bem antes do minuto 45.
- **`resolvePenalty` (`resolveAction.ts`) ganhou um 4º parâmetro opcional `choice?: PenaltyChoice`.** Omitido (todo chamador não-interativo: amistoso, duelo, copa, e o próprio pênalti do ADVERSÁRIO numa partida interativa), o caminho é **byte-idêntico** ao original — mesma fórmula, mesma contagem de chamadas de `rng()` — preservando o determinismo de todo seed já existente. Só quando o jogador real escolhe canto+força é que entra o caminho novo: o goleiro "adivinha" um canto de forma independente (RNG própria), acertar/errar o palpite pesa bastante na chance de defesa, e força "Forte" tem um bônus de qualidade com uma chance própria de mandar pra fora.
- **Escopo deliberadamente cortado: só pausa entre os minutos 1-70.** `runSecondHalf` (71'+ e acréscimos) sempre resolve pênalti inline, nunca pausa — tornar o trecho final resumível também exigiria que `MatchResult` (não só `MatchHandle`) tivesse uma variante "pausado", e o cálculo de acréscimos já depende da lista COMPLETA de eventos. Escopo desproporcional pra um caso raro de fim de jogo — ver Risco novo no registro.
- **`playCareerMatch` ganhou `resolvePenaltyDecision`, mais um hook opcional, mesmo padrão dos dois hooks táticos.** A orquestração interna virou um laço (`drainPendingPenalties`): resolve o pênalti pendente (perguntando só quando é o lado do JOGADOR — o pênalti do adversário nunca pergunta, resolve sozinho) e continua rodando até o próximo checkpoint, repetindo se outro pênalti acontecer antes de chegar lá. O trecho 46'-70' agora sempre passa pelo caminho pausável quando existe um hook de pênalti, mesmo sem `resolveLateGameTactic` — senão um pênalti nessa janela nunca teria chance de pausar.
- **Nova UI**: `discord/ui/penaltyDecisionCard.ts` — 6 botões (3 cantos × 2 forças) em 2 linhas, mesmo padrão de customId prefixado + collector escopado à mensagem que os cards táticos já usam. Timeout de 5 minutos cai pro meio/colocado (documentado no rodapé do card, mesmo padrão da tática).

**O que foi validado de verdade:** os 405 testes existentes passando sem nenhuma alteração — prova de que `pauseOnPenalty` omitido (todo chamador não-interativo) é 100% comportamento antigo. 9 testes novos no motor (seeds achados por força bruta até produzirem um pênalti pro lado desejado — mesma técnica já usada pra seeds de gol/lesão no projeto): pausa exatamente na hora certa sem processar nada depois, `resumeFirstHalf` é no-op seguro quando não há nada pendente, pênalti do adversário resolve igual com ou sem escolha, pênalti na janela 46-70 pausa corretamente depois de retomar do intervalo, e a escolha de canto/força é uma alavanca real (mesma seed, escolhas diferentes divergem). 3 testes novos no serviço (hook chamado com o contexto certo; escolha diferente diverge; `/jogar-carreira` renderizado via script throwaway). `npx tsc --noEmit`, `npx eslint .`, `npx vitest run` (414 passando) e `npm run build` — todos limpos.

## Adenda (follow-up) — Substituição interativa: mesma arquitetura de pausa do pênalti, aplicada ao cansaço

Pedido direto do usuário: "faz a substituição interativa também" — a outra metade do pedido original de "mais decisões ao vivo na partida" que a adenda de Pênalti interativo tinha deixado de fora por escopo, mais um pedido explícito de revisão de bugs/cortes no código recente (resultado no fim desta adenda e nos Riscos #61-62 do registro).

Decisões de arquitetura — deliberadamente simétricas às do pênalti interativo, mesmo mecanismo aplicado a um gatilho diferente:

- **`substitution.ts` foi refatorado, não reescrito**, pra expor as peças que a pausa precisa: `pickReplacement` e `applySubstitutionWithReplacement` (antes privadas) viraram exportadas, e um novo `findTiredPlayer` (versão pura/somente-leitura da busca por jogador cansado, sem aplicar nada) foi extraído de `maybeSubstituteTired`. `substituteInjured`/`sendOff`/`maybeSubstituteTired` mantêm exatamente o mesmo comportamento observável de antes — confirmado por grep (nenhum teste referenciava os internos antigos diretamente) e pela suíte de testes inteira passando sem alteração.
- **`MatchSimState.pendingSubstitution`** (`game/domain/state.ts`) espelha `pendingPenalty`: quando `MatchOptions.pauseOnSubstitution` está ligado, `resolvePhase.ts` captura `{ minute, side, outgoing, benchOptions }` (jogadores inteiros, não só ids — mesma razão do `taker`/`gk` do pênalti: evita relookup se o estado mudar antes da retomada) em vez de aplicar a troca na hora.
- **`pendingPenalty` e `pendingSubstitution` são mutuamente exclusivos DENTRO de um mesmo minuto** (o laço de checagem de cansaço em `resolvePhase` só roda `if (!state.pendingPenalty)`), mas não entre minutos diferentes — por isso `playCareerMatch.ts` trocou o antigo `drainPendingPenalties` por um `drainPendingDecisions` unificado, que resolve qualquer um dos dois pendentes num laço só (`while (pendingPenalty || pendingSubstitution)`), já que um pode aparecer logo depois do outro ser resolvido, antes do próximo checkpoint.
- **`resumePendingSubstitution`** (novo, exportado de `simulateMatch.ts`) usa a escolha explícita se for um id válido do banco; caso contrário (omitida, inválida, ou um timeout do lado Discord retornando `""`) cai pro mesmo `pickReplacement` que a substituição automática sempre usou — garantindo que "não decidir" e "decidir automaticamente" continuam sendo o mesmo caminho de código.
- **Mesmo corte de escopo do pênalti: só pausa entre os minutos 1-70** (`runSecondHalf` sempre resolve inline), pela mesma razão (`MatchResult` não tem variante "pausado").
- **Achado real durante a validação, não hipotético — o pedido de "verifica bugs/cortes" encontrou algo concreto:** o gatilho de substituição por cansaço (`TIRED_THRESHOLD = 35`) é, na prática, quase inatingível sob as constantes de dreno de estamina atuais — confirmado simulando 2000 partidas sintéticas até o minuto 70 (0 pausas, incluindo com estilo AGGRESSIVE dos dois lados) e checando a estamina mínima ao final de 500 partidas completas (63.3, nunca perto de 35). Isso significa que os testes do motor para este ponto de pausa **não puderam usar busca de seed por força bruta** (a técnica usada em toda outra parte deste projeto, inclusive no pênalti) — porque o cenário não acontece organicamente em nenhuma quantidade razoável de tentativas. Os testes novos em vez disso setam a estamina diretamente em `MatchHandle.state` antes de retomar a simulação, usando exatamente o ponto de mutação que a própria documentação de `MatchHandle` já estabelece como legítimo pra controle de teste interativo. Ver Risco #61 no registro pro detalhe completo e a decisão consciente de não recalibrar as constantes de dreno nesta leva (seria um ajuste de balanceamento "no olho", mesma categoria dos Riscos #12/#20/#29/#58 — fora do que foi pedido). Um segundo achado (Risco #62): o fallback de `pickReplacement` pra "sem cobertura na mesma posição" pega o primeiro do banco, que é sempre o goleiro reserva (`BENCH_FORMATION` começa com `GK`) — pode substituir um lateral cansado pelo goleiro reserva jogando de linha. Pré-existente, não corrigido nesta leva pela mesma razão (misturaria mudança de comportamento com o refactor "zero mudança"). Um terceiro achado (Risco #63) foi introduzido NESTA MESMA leva e corrigido antes do commit: a primeira versão da guarda de mútua exclusão em `resolvePhase.ts` envolvia tanto o ramo interativo quanto o automático em `if (!state.pendingPenalty)`, quando só o ramo interativo precisa dela — o efeito seria pular silenciosamente a checagem de cansaço automática em qualquer minuto que também tivesse um pênalti pendente, mesmo pra chamadores que nunca habilitaram `pauseOnSubstitution`. Corrigido movendo a guarda pra dentro do `if (pauseOnSubstitution)`, com um teste de regressão que prova o bug revertendo a correção temporariamente antes de restaurá-la.
- **Nova UI**: `discord/ui/substitutionDecisionCard.ts` — um botão por reserva disponível (até 10, em linhas de 5), rótulo "Nome (Posição)". Timeout de 5 minutos cai pro reserva mais fresco da mesma posição (mesmo `pickReplacement` automático), documentado no rodapé do card.

**O que foi validado de verdade:** os 414 testes existentes passando sem nenhuma alteração — prova de que `pauseOnSubstitution` omitido é 100% comportamento antigo. 9 testes novos no motor: como o gatilho é inatingível por seed (ver acima), usam estamina forçada em `MatchHandle.state` pra provar pausa exata na janela certa com o formato certo do estado pendente, escolha explícita realiza a troca exata mesmo quando não é o pick automático, escolha omitida/inválida caem no mesmo pick automático determinístico, substituição do adversário resolve igual com ou sem escolha, o caminho automático (sem pausa) continua realizando a troca de verdade, e um teste de regressão dedicado pro bug do Risco #63 (a guarda de `pendingPenalty` só vale pro ramo interativo — provado revertendo a correção temporariamente e confirmando que o teste falha antes de restaurar). 1 teste novo no serviço confirmando que omitir o hook é byte-idêntico ao baseline (comparando placar e sequência de eventos, não o `.result` inteiro — ids de time/jogador vêm de `randomUUID()` por instância de repositório, não do seed compartilhado). `npx tsc --noEmit`, `npx eslint .`, `npx vitest run` (423 passando) e `npm run build` — todos limpos.

## Adenda (follow-up) — Cartas ilustradas: imagem gerada por código, não foto externa

Pedido direto do usuário: "quero que as cartas seja visível com foto pois é o desfecho do bot" (destaque do produto). Como todo jogador do catálogo é fictício (`cards/domain/catalog.ts`, mesma disciplina de nomes inventados do `career/domain/clubNaming.ts`), não existe foto real pra usar — perguntado ao usuário via `AskUserQuestion` entre gerar uma carta ilustrada completa (estilo FUT, moldura/atributos desenhados) vs. um ícone simples por posição/raridade; escolhida a primeira opção.

Decisões de arquitetura:

- **Nova dependência nativa: `@napi-rs/canvas`.** Único pacote de geração de imagem no projeto até aqui (nenhum `canvas`/`sharp`/SVG-to-PNG existia). Escolhido especificamente por distribuir binário pré-compilado por plataforma via npm (sem Cairo/Pango de sistema, ao contrário do `node-canvas` clássico) — confirmado instalando e rodando de verdade neste ambiente (linux-x64) antes de decidir a dependência, não só supondo que funcionaria.
- **`discord/ui/cardImage.ts`** é o único módulo novo: `renderCardImage(input): Buffer` é uma função pura (sem I/O, sem estado) que desenha a carta inteira em memória e devolve um PNG — moldura com gradiente por raridade (cinza/azul/roxo/dourado/arco-íris, espelhando `CARD_RARITY_EMOJI`), OVR + posição no topo, um "slot de foto" circular com silhueta desenhada via paths (círculo pra cabeça + arco pros ombros — não uma imagem externa, já que não há foto real pra compor), nome com auto-fit de fonte (reduz o tamanho até caber, pra nomes longos como "O Fenômeno de Vale Verde"), grid de atributos 2×3 reaproveitando `CORE_ATTRIBUTE_SHORT_LABELS`/`GOALKEEPER_ATTRIBUTE_SHORT_LABELS` (já existentes em `player/domain/labels.ts` — as chaves de atributo das cartas usam os mesmos nomes de campo dos jogadores) e uma faixa de habilidade quando a carta tem uma. `buildCardAttachment(input, filename)` embrulha isso num `AttachmentBuilder` do discord.js, pronto pra ir em `files:` ao lado de um `MediaGalleryBuilder` no Components V2.
- **Achado real durante a validação (não hipotético): emoji não renderiza neste ambiente.** A primeira versão da faixa de habilidade usava "✨" desenhado com `fillText` — confirmado visualmente (gerando um PNG de exemplo e inspecionando via leitura de imagem) que aparecia como um retângulo vazio ("tofu"), porque as fontes disponíveis no container (Liberation/DejaVu/FreeFont, via fontconfig) não têm glifos de emoji colorido. Trocado por texto simples ("HABILIDADE: X", sem emoji) — funciona em qualquer ambiente, independente de fonte de emoji instalada. Ver Risco novo no registro.
- **Segundo achado real: sobreposição de layout entre a faixa de habilidade e a última linha do grid de atributos**, também só percebido inspecionando visualmente o PNG gerado (não algo que `tsc`/`eslint`/testes de buffer detectariam sozinhos) — corrigido ajustando o espaçamento vertical antes de escrever qualquer teste permanente.
- **Só `/carta` (1 carta) e `/abrir-pacote` (até `cardCount`, hoje 3) ganharam imagem — `/colecao` continua texto.** Não é só uma questão de volume "razoável": o catálogo inteiro tem 13 cartas fixas (`CARDS` em `catalog.ts`), e o Discord limita mensagens a 10 anexos — um colecionador com o catálogo completo já excederia esse limite numa `/colecao` com imagem por carta. Documentado aqui em vez de tentar contornar (paginação, múltiplas mensagens) sem pedido explícito.
- **Filename de anexo por ÍNDICE, não por id da carta** (`card-0.png`, `card-1.png`...), porque um pacote pode sortear a MESMA carta mais de uma vez — nomes de anexo precisam ser únicos dentro de uma mensagem, e o id da carta não garante isso.
- **Sem cache/armazenamento de imagem** — cada card é gerado on-demand a cada chamada de comando (nenhuma imagem é persistida em disco/banco/CDN). Estimativa aproximada de custo (não um benchmark instrumentado): o teste que renderiza as 11 posições de linha em sequência levou ~300ms no total nesta máquina, uma ordem de grandeza de ~20-30ms por carta, o suficiente pra ser imperceptível num comando síncrono de usuário único. Aceitável por ora; se o volume de uso crescer a ponto de a geração repetida da mesma carta pesar de verdade, cachear os PNGs das 13 cartas fixas do catálogo (elas nunca mudam) seria a otimização natural — não implementado agora por falta de necessidade medida. Ver Risco novo no registro.
- **`Card.imageUrl` (schema, já existente desde antes) continua sem uso.** Essa coluna teria feito sentido pra uma abordagem "foto/URL estática por carta" — como a decisão foi gerar a imagem por código em vez de armazenar uma URL, essa coluna permanece morta. Não removida (fora de escopo mexer em schema por uma decisão de UI), mas registrada aqui pra quem for procurar por que existe sem nenhum leitor.

**O que foi validado de verdade:** 7 testes novos (`tests/unit/discord/ui/cardImage.test.ts`, primeiro teste de um módulo `discord/ui/*` neste projeto — os demais são adapters finos de comando, tradicionalmente não testados diretamente) — PNG válido (assinatura de magic bytes) pra cada uma das 5 raridades, atributos de goleiro usados corretamente pra posição GK, não lança exceção pra nome/habilidade extremamente longos nem pra um mapa de atributos incompleto, cobre as 11 posições de linha, e `buildCardAttachment` embrulha o buffer certo com o nome certo. Validação visual real (não só buffer/tamanho): 5 cartas de exemplo (uma por raridade, incluindo GK e nome longo) renderizadas via script throwaway e inspecionadas via leitura de imagem antes de escrever qualquer teste — foi assim que os dois achados acima (emoji tofu, sobreposição) foram descobertos, não por acaso. `npx tsc --noEmit`, `npx eslint .`, `npx vitest run` (430 passando) e `npm run build` — todos limpos.
