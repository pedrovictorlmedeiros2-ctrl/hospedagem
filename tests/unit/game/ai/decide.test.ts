...
import { decideAttackAction, decideDefenseReaction, type DecisionContext } from "../../../../src/game/ai/decide.js";
...
function countAttackActions(context: DecisionContext, style: Parameters<typeof decideAttackAction>[1], seed: string, trials = 400) {
...
function countDefenseReactions(context: DecisionContext, style: Parameters<typeof decideDefenseReaction>[1], seed: string, trials = 400) {
  const rng = createRng(seed);
...
describe("decideAttackAction", () => {
  it("shoots far more often deep in the opponent's box than in its own half", () => {
    const deep = countAttackActions(baseContext({ attackerProgress: 0.95 }), "TACTICAL", "deep-seed");
    const shallow = countAttackActions(baseContext({ attackerProgress: 0.1 }), "TACTICAL", "shallow-seed");
  ...
  it("holds the ball more when tired than when fresh", () => {
    const fresh = countAttackActions(baseContext({ attackerStamina: 95 }), "TACTICAL", "stamina-seed-a");
    const tired = countAttackActions(baseContext({ attackerStamina: 20 }), "TACTICAL", "stamina-seed-b");
  ...
  it("gets more desperate (shoots/dribbles more, holds less) when losing late", () => {
    const calm = countAttackActions(baseContext({ scoreDiff: 0, timeRemaining: 60 }), "TACTICAL", "calm-seed");
    const desperate = countAttackActions(baseContext({ scoreDiff: -1, timeRemaining: 5 }), "TACTICAL", "desperate-seed");
...
describe("decideDefenseReaction", () => {
  ...
  it("recovers more than it presses when exhausted", () => {
    const fresh = countDefenseReactions(baseContext({ defenderStamina: 95 }), "AGGRESSIVE", "rec-seed-a");
    const exhausted = countDefenseReactions(baseContext({ defenderStamina: 15 }), "AGGRESSIVE", "rec-seed-b");
